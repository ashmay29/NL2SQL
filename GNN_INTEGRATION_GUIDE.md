...
This guide explains how to integrate your trained GNN model with the NL2SQL system for enhanced schema understanding and query embedding generation. The system is designed to accept external GNN models via HTTP API and gracefully fall back to Sentence Transformers when the GNN service is unavailable.

### Architecture Flow

```
User Query (NL) + Optional Database Upload (UI)
    ↓
[Frontend] → React UI with database upload component
    ↓
[Data Ingestion API] → CSV/Excel/DB Tables → Schema Extraction
    ↓
[Schema → Graph Conversion] → Nodes (tables/columns) + Edges (relationships)
    ↓
[Sentence Transformer] → Text Embeddings (384-dim)
    ↓
[GNN Model Inference] → Graph-aware Embeddings (512-dim) [EXTERNAL SERVICE]
    ↓
[Redis Cache] → Fast retrieval for subsequent queries
    ↓
[Prompt Builder] → Enhanced prompt with relevant schema context
    ↓
[LLM (Gemini)] → Intermediate Representation (IR) Generation
    ↓
[IR Validator] → Schema validation & error detection
    ↓
[IR Compiler] → MySQL SQL Generation
    ↓
[Response] → SQL + Confidence + Explanations
```
...
#### **Option A: Via UI (Recommended for End Users)**

1. Navigate to `http://localhost:3000/playground`
2. Click "Upload Database" button
3. Select your file (CSV, Excel, JSON, or .db)
4. System automatically:
   - Extracts schema
   - Computes fingerprint
   - Generates embeddings (GNN or fallback)
   - Caches in Redis
...
4. View generated SQL, confidence, and explanations
...
5. IR (Intermediate Representation) generated by Gemini
6. IR validated against schema
7. IR compiled to MySQL SQL
8. SQL returned with confidence and explanations
...
The frontend (`frontend/src/`) has been updated to support database uploads:

#### **1. Home Page** (`pages/Dashboard.tsx`)
- Professional hero section explaining NL2SQL
- Visual demonstration of NL → SQL transformation
- Feature highlights and statistics
- CTA button to playground

#### **2. Playground** (`pages/NLSQLPlayground.tsx`)
- **Database Upload Section**:
  - Toggle button to show/hide upload
  - Drag-and-drop file upload area
  - Supports: `.sql`, `.csv`, `.json`, `.db` files
  - File preview with name and size
  - Remove file functionality
  - Info message about current database source
- **Query Input Section**:
  - Natural language query input
  - Mode selection (Direct vs Step-by-Step)
  - Generate SQL button
- **Results Display**:
  - Generated SQL with syntax highlighting
  - Confidence score and execution time
  - Clarifications and suggestions
  - IR (Intermediate Representation) view in step-by-step mode

### **Navigation**

Simplified navigation (`components/Layout.tsx`):
- **Home**: Landing page with hero section
- **Playground**: Main NL2SQL interface with upload

Removed pages:
- ~~Schema Explorer~~ (functionality integrated into playground)
- ~~Embeddings Manager~~ (handled automatically in backend)
...
1. User visits home page (/)
   ↓
2. Clicks "Try Playground" button
   ↓
3. (Optional) Uploads database file
   ↓
4. Enters natural language query
   ↓
5. Clicks "Generate SQL"
   ↓
6. Views SQL + confidence + explanations
   ↓
7. Can provide feedback or download SQL
```
...
8. ✅ **Frontend UI** - Database upload component in playground
9. ✅ **IR Validation** - Schema validation with CTE support
10. ✅ **IR Compilation** - Handles aggregate expressions in ORDER BY
...
- **UI Enhanced**: Professional home page + database upload in playground
- **Robust Validation**: IR validator handles CTEs, aggregates, and complex queries

### **Next Steps to Full Integration**

1. **Train your GNN model** on your schema graphs
2. **Deploy GNN model server** with required endpoints:
   - `POST /infer/schema` - Generate schema embeddings
   - `POST /infer/query` - Generate query embeddings
   - `POST /similarity/top_k` - Find relevant nodes
...
6. **Generate embeddings**:
   ...
   ```
7. **Monitor performance** and accuracy improvements

### **Key Integration Points**

For external GNN model developers:

1. **Input Format**: Your GNN server receives:
   - Graph representation (nodes + edges)
   - Schema fingerprint
   - Query text (for query embeddings)

2. **Output Format**: Your GNN server returns:
   - Embeddings as JSON: `{"node_id": [float, ...], ...}`
   - Dimension: 512 (configurable via `EMBEDDING_DIM`)
...
4. **Fallback Behavior**: If your GNN server is unavailable, the system automatically falls back to Sentence Transformers, ensuring uninterrupted service.
